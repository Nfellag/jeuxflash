...
if(isset($_GET['token']) && $_GET['token']!= ''){
    $stmt = $bdd -> prepare("SELECT email FROM utilisateurs WHERE token = ?");
    $stmt -> execute([
        $_GET['token']
    ]);
    $email = $stmt -> fetch();
...
if (isset($_POST["update_password"])) {
    ...
    $stmt = $bdd->prepare("UPDATE utilisateurs SET password = :password, token=NULL WHERE email = :email");
    ...
    if ($result) {
        ...
        header('Location: http://localhost/jeuxflash/gameflash/pages/LoginPages.php');

        // et envoyer un mail pour notifier la modification du mot de passe.
        $subject = "Votre mot de passe a été modifié";

        $url = "http://localhost/jeuxflash/gameflash/pages/token/update_password.php?";
